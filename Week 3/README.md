...
way up to a neural network. There is **no football this week**. Instead you work
...
models**, each one building on the last.
...
| 2 | `02-monte-carlo-simulation.ipynb` | **Monte Carlo simulation.** Play out thousands of random futures to find the real chance of hitting a savings goal, and how one habit changes the odds. |
| 3 | `03-time-series-and-stationarity.ipynb` | **Time series and stationarity.** Why prices drift and returns do not, plus differencing and autocorrelation, the most important idea in forecasting. |
...
| 6 (bonus) | `06-bonus-neural-network.ipynb` | A small **neural network** (the family of models behind modern AI) taking on a near-random market. |
...
- `monthly_budget.csv`, 48 months of one teenager's income and spending.
- `daily_stock.csv`, about three years of daily prices for a made-up company, Coral
  Reef Games (ticker `REEF`).
- `variable_dictionary.md`, every column explained in plain words, plus the real
...
Cup matches). Week 3 reuses the same models you met there (logistic regression,
decision trees, and neural networks), so seeing them on football first makes the
